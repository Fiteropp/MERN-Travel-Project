...
export default function AdminListUsers() {
    ...
    const { showAlert } = useAlert(); // Use global alert function
    ...
    const EditUser = async (UserID) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}api/admin/edituserrole/${UserID}`, {
                method: "POST",
                credentials: "include",
        ...
        <div>
                    ...
                    <Box key={users._id} sx={{ flexGrow: 1, width: '100%' }} >
                        <Grid2 container spacing={1} columns={12} sx={{ width: '100%' }} className="listUsersElementContainer">
                            ...
                            <Grid2 size="grow" className="listUsersButtonContainer">
                                <div>
                                    <Button className="edit-button" variant="outlined" onClick={() => handleClickOpen(users._id)}>
            ...
            >
                ...
                <DialogActions>
                    ...
                    <Button variant="contained" onClick={() => EditUser(selectedUserId)}>Confirm</Button>
                </DialogActions>
            </Dialog>
        </div>
    )
